...
class RocketDetailViewModel : ViewModel() {
    private val launchesRepository: LaunchRepository =
        LaunchRepositoryImpl(ApiUtil.launchApi, DatabaseUtil.launchDao)
    private val rocketRepository: RocketRepository =
        RocketRepositoryImpl(ApiUtil.rocketApi, DatabaseUtil.rocketsDao)
    ...
    private fun groupAndSort(list: List<Launch>?): Map<Date?, List<Launch>>? {
        ...
        return list?.sortedBy { launch -> launch.launchDateUtc }
            ?.groupBy { launch ->
                launch.launchYear.let { dateFormat.parse(it.orEmpty()) }
            }
    }
